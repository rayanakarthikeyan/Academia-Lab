...
The tester opens **Faculty drafts** and refreshes to load the latest saved copies. The standard IDE uses the saved instructions, source and expected output, alongside matching interactive lab tools. It supports input and execution without final submission. Telemetry carries the draft ID and saved-version timestamp, with the server-derived tester flag. Regular students use these tools only after the faculty publishes the corresponding lab assignment, and submit their report and activity evidence through that assignment.
